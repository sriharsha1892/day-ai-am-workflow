...
export function initializeServer(server) {
  ...
  server.registerTool(
    ...
    {
      description:
        'Production Day AI write. action one of: org-link | org-create | org-update-tags | opportunity-create | opportunity-update-stage | person-dedupe-check | person-create | contact-update-tags | action-create | draft-create | review-context. Requires AM approval upstream. Idempotency key reused on retry — never creates duplicates. Attributed to the signed-in AM. For review-context, the note body MUST be in payload.content (with a short payload.summary) — an empty body is rejected. For opportunity-create, payload may include `stageId` (UUID from workspace-ids.json), `title`, `ownerEmail`, and `customProperties` (array of {propertyId, value, reasoning?} — picklist values must be option UUIDs, not display names). For opportunity-update-stage, payload requires `opportunityId` + `stageId`. For org-update-tags, payload requires `canonicalDomain` + `customProperties` (W4 cleanup scripts). For contact-update-tags, payload requires `contactEmail` + `customProperties` (W4 contact cleanup).',
      inputSchema: {
        action: z.enum([
          'org-link',
          'org-create',
